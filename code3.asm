...
score: db 'SCORE:', 0
screen: db 'Enter Y to play again. Or N to exit.', 0
scoreflag: db 0
...
merawaqt: 	
    ...
    ;these checks are for that after 30 seconds one life will go down

    ;if 30 seconds arenot completed go on whaat is going on	
	cmp word [seconds],30
	jne gooon

    ;if check is 0 then go on doing the thing and if 1(30s completed) then decrement life
	cmp word [check],0
	jne gooon

	cmp word [cs:tickcount],16
	jne gooon
 
    ;decremeting the life
	dec word [life]
...
gooon:	
	...
	push word 120   ;di to print string
	mov ax,time     ;string
	push ax
	push word 11    ;length of the string

	call printstring

	inc word [cs:tickcount]; increment tick count
   	cmp word [cs:tickcount],18     ;1 sec = 18 tickcounts
	je incre                       ;if 18 tickcounts then increase time
	jne ee                         ; if not equal then go and print number

incre:	
	inc word [seconds]             ;incrementing second
	mov word [cs:tickcount],0

	cmp word [seconds],60          ; if 60 seconds are gone then increment minute
	je min
	jne ee

min:	
    inc word [minutes]           ;incrementing the minute
	mov word [seconds],0         ;again moving seconds to 0

ee:	
    push word 134
	push word [minutes]
	call printnumber ; print tick count	
...
waqt: 
	...
	mov ax,[es:8*4]     ;int 8---> time
	
	;storing old interrupt service routine to registers
	mov [oldsir],ax
	...
	mov word [es:8*4], merawaqt; store offset at n*4
...
;this will work to unhook the interrupt
busab:	
    pusha
...
printstring: 	
		...
		mov ah, 0x0C ; normal attribute fixed in al

    nextcha: 	mov al, [si] ; load next char of string
		mov [es:di], ax ; show this char on screen
		add di, 2 ; move to next screen location
		add si, 1 ; move to next char in string
		loop nextcha ; repeat the operation cx times
...
printnumber: 	
   ...
   nextdig: 	
		...
		inc cx ; increment count of values
		cmp ax, 0 ; is the quotient zero
		jnz nextdig; if no divide it again
		mov di, [bp+6]
  ...
  nextpo: 	
		...
		mov [es:di], dx ; print char on screen
		add di, 2 ; move to next screen location
		loop nextpo ; repeat for all digits on stack
		pop di
		pop dx
		pop cx
		pop bx
...
main:
	...
	; mov ax, 0x1E20  ;attribute +11
	; push ax
	; mov ax, screen ;string +8
	; push ax
	...
	endend:
	call busab    ;ending time


	call khatumsab   ;game over screen
